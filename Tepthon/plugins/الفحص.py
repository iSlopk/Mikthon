...
async def amireallyalive(event):
    ...
    start = datetime.now()
    zedevent = await edit_or_reply(event, "**يتِم فحـص تنصيبـك لـ 𝗠𝗶𝗸𝘁𝗵𝗼𝗻𔘓  . .**")
    end = datetime.now()
    ...
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "**- بوت ميكثون  𝗠𝗶𝗸𝘁𝗵𝗼𝗻 يعمـل بنجـاح 🌿 ..**"
...
**{Z_EMOJI} قنـاتنا :** [اضغـط هنـا](https://t.me/slopk)"""
...
async def amireallyialive(event):
    ...
    reply_to_id = await reply_id(event)
    Z_EMOJI = gvarstatus("ALIVE_EMOJI") or "✾╿"
    zed_caption = "**- بوت ميكثون  𝗠𝗶𝗸𝘁𝗵𝗼𝗻 يعمـل بنجـاح 🌿 .. **\n"
