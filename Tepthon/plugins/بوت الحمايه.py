...
async def check_edit_media(event):
    ...
    if event.is_group: #Write Code By T.me/zzzzl1l
        ...
        if not admin and not creator:
            return
    zed_dev = (1332671807, 658351825, 1895219306, 12665030, 658351825)  #Write Code By T.me/zzzzl1l
    zelzal = event.sender_id
    malath = zedub.uid
    hhh = event.message.text
    zed_id = event.chat_id
...
async def _(event):
    if not event.is_private:
        ...
        if not admin and not creator:
            return
    # All Rights Reserved for "Zed-Thon - ZelZal" "زلـزال الهيبـه"
    zed_dev = (1332671807, 65951825, 18919306, 12605030, 65951825)
    malath = zedub.uid
...
async def _(event):
    ...
    zed_dev = (1332671807, 552220000, 6583951825)
...
async def _(event):
    if not event.is_private:
        ...
        if not admin and not creator:
            return
    # All Rights Reserved for "Zed-Thon - zthon" "زلـزال الهيبـه"
    zed_dev = (1332671807, 65851825, 189519306)
...
async def zelzal(zed):
    ...
    if con != "طرد":
        ...
        async for user in zed.client.iter_participants(zed.chat_id):
            if user.bot:
                ...
                await sleep(0.5)
        if del_u > 0:
            del_status = f"🛂**┊كشـف البـوتات -** 𝑴𝑰𝑲𝑻𝑯𝑶𝑵\
    ...
    if del_a > 0:
        del_status = f"❇️**┊طـرد البـوتات -** 𝑴𝑰𝑲𝑻𝑯𝑶𝑵\
