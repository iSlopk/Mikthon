...
async def _(zed):
    await edit_or_reply (zed, "𓆰 [𝙎𝙊𝙐𝙍𝘾𝞝 𝗠𝗶𝗸𝘁𝗵𝗼𝗻 - 𝘿𝙀𝙇𝙀𝙏𝙀](t.me/TepThon) 🗑♻️𓆪\n**𓍹━─━─━─━─𝗠𝗶𝗸𝘁𝗵𝗼𝗻─━─━─━─━𓍻**\n\n **✵│رابـط الحـذف ↬** https://telegram.org/deactivate \n\n\n **✵│بـوت الحـذف  ↬** @LC6BOT ")
...
async def permalink(mention):
    ...
    if user.id in zel_dev:
        return await edit_or_reply(mention, f"**╮ ❐ لك دي . . هـذا المطـور  ❏╰**")
    if user.id == 1332671807 or user.id == 1332671807 or user.id == 1332671807:
...
ZelzalTHS_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝗠𝗶𝗸𝘁𝗵𝗼𝗻 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر التحشيش](t.me/TepThon) 𓆪\n\n"
...
async def permalink(mention):
    ...
    if user.id == 1332671807 or user.id == 1332671807 or user.id == 1332671807:
        return await edit_or_reply(mention, f"**╮ ❐ لك هـذا مايـك . . شما حجيت ماكـدر اوصفـه 🙊💘 ❏╰**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
